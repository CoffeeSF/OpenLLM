...
mkdir(root.."/config")
local f=assert(io.open(root.."/config/rack.lua","w")); f:write("return { coordinator = ",string.format("%q",state.coordinator),", workers = { [1] = ",string.format("%q",state.workers[1]),", [2] = ",string.format("%q",state.workers[2]),", [3] = ",string.format("%q",state.workers[3])," }, context = 128, timeout = 30 }\n"); f:close()
