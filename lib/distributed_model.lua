...
function M.open(path)
  ...
  local h=storage.read_exact(f,HEADER); if h:sub(1,4)~="OCTP" or u(h,5)~=1 then error("invalid OCTP shard") end
  ...
  local size=f:seek("end"); if size<o then error("truncated shard") end
  return setmetatable({file=f,p=p,s=s},{__index=M})
end
function M:close() self.file:close() end
function M:norm(kind, layer, out)
  ...
  self.file:seek("set",o); local b=storage.read_exact(self.file,256); for i=1,64 do out[i]=storage.f32le(b,(i-1)*4+1) end
end
function M:matvec(sec, row_base, input, output, count)
  local w=sec.columns
  for r=0,count-1 do
    self.file:seek("set",sec.offset+(row_base+r)*(w+4)); local scale=storage.read_f32(self.file); local b=storage.read_exact(self.file,w); local sum=0
    for c=1,w do sum=sum+storage.signed_byte(b,c)*input[c] end
    output[r+1]=sum*scale
  end
end
function M:embedding(token,out) self:matvec(self.s.emb,token,{[1]=1},out,0); self.file:seek("set",self.s.emb.offset+token*68); local sc=storage.read_f32(self.file); local b=storage.read_exact(self.file,64); for i=1,64 do out[i]=storage.signed_byte(b,i)*sc end end
return M
